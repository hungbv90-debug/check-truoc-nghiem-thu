...
def parse_block_attrs(raw_content):
    ...
    # 1. First find box_code among all parts
    for part in parts:
        val = part.strip()
        if val.startswith('P') or val.startswith('p'):
            if len(val) > 1 and val[1].upper() in ['C', 'H', 'S', 'M', 'T', 'O', 'B', 'F', 'D', '1', '2', '3', '4', '5', '6', '7', '8', '9']:
                val = val[1:]
        if any(val.upper().startswith(x) for x in ['H', 'C', 'M', 'T', 'O', 'B', 'F', 'D']) and re.search(r'P\d{2,4}\.\d{4}', val):
            box_code = val
            break
            
    # If no standard box code found, fall back to first part containing P\d.\d
    if not box_code:
        for part in parts:
            if re.search(r'P\d{2,4}\.\d{4}', part):
                val = part.strip()
                if val.startswith('P') or val.startswith('p'):
                    if len(val) > 1 and val[1].upper() in ['C', 'H', 'S', 'M', 'T', 'O', 'B', 'F', 'D', '1', '2', '3', '4', '5', '6', '7', '8', '9']:
                        val = val[1:]
    ...
    # 2. Look for capacity text containing 'PORT' or other capacity patterns
    # We scan all parts to make sure we don't miss anything.
    for part in parts:
        val = part.strip()
        # Clean leading P for comparison
        val_clean = val
        if val_clean.startswith('P') or val_clean.startswith('p'):
            if len(val_clean) > 1 and val_clean[1].upper() in ['C', 'H', 'S', 'M', 'T', 'O', 'B', 'F', 'D']:
                val_clean = val_clean[1:]
                
        if val_clean == box_code:
            continue
            
        if 'SP' in val.upper() or '1:' in val.upper() or '2X' in val.upper() or 'CAP' in val.upper() or 'PORT' in val.upper():
            if capacity_text:
                capacity_text = f"{capacity_text} {val}"
            else:
                capacity_text = val

    # If capacity_text is still empty, scan all parts for a simple port count, e.g. "8port" or "8 port"
    if not capacity_text:
        for part in parts:
            if re.search(r'\d+\s*port', part, re.IGNORECASE):
                capacity_text = part
                break

    # 3. If we still don't have a box_code, take the first part as a fallback
    if not box_code and parts:
        box_code = parts[0]
...
def extract_numeric_capacity(cap_text):
    ...
    # Explicit port parsing: e.g. "4PORT" or "8 PORT" -> 4 or 8
    port_match = re.search(r'(\d+)\s*PORT', cap_text_clean)
    if port_match:
...
def extract_gpon_topology(dxf_path):
    ...
    def process_layout(layout, layout_name="Model"):
        ...
        for text_entity in layout.query('TEXT MTEXT'):
            ...
            if re.search(r'P\d{2,4}\.\d{4}', content):
                ...
                if block_id_match:
                    name_match = re.search(r'([A-Z]*P\d{2,4}\.\d{4}\s*/[A-Z]+)', content)
                    name = name_match.group(1).replace(" ", "") if name_match else content.split('\\')[0].replace(" ", "").replace(";", "").replace("{", "")
                    
                    # Clean leading 'P' if it's a paragraph break code merged with prefix (e.g. PHCMP -> HCMP)
                    if name.upper().startswith('P') and len(name) > 1 and name[1].upper() in ['C', 'H', 'S', 'M', 'T', 'O', 'B', 'F', 'D', '1', '2', '3', '4', '5', '6', '7', '8', '9']:
                        name = name[1:]
    ...
    def find_nearby_attributes(insert_pt, lattrs, max_dist=15.0):
        ...
        for t in lattrs:
            ...
            if dist <= max_dist:
                ...
                if 'SP' in val.upper() or '1:' in val.upper() or '2X' in val.upper() or 'CAP' in val.upper() or 'PORT' in val.upper() or ':' in val:
    ...
    for b in blocks_info:
        ...
        val_data = {
            ...
            'capacity_num': extract_numeric_capacity(cap_text)
        }
