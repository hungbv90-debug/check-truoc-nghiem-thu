...
class QALogic:
    ...
    def _safe_num(self, val: Any) -> float:
        ...
        # Safeguard: if the string looks like a station/box name, do NOT parse it as a capacity number.
        # Examples: "HCMP758.0002/HO", "P758.0002", etc.
        s_upper = s.upper()
        if '/' in s_upper and any(suffix in s_upper for suffix in ['/HO', '/TO', '/TU', '/FO', '/MO', '/SO', '/CO']):
            return 0.0
        if re.search(r'[A-Z]*P\d{2,4}\.\d{4}', s_upper):
            return 0.0
    ...
    def check_doi_tuong(self, df_import: pd.DataFrame, df_bbnt_dt: pd.DataFrame) -> pd.DataFrame:
        ...
        """
    def check_doi_tuong(self, df_import: pd.DataFrame, df_bbnt_dt: pd.DataFrame, df_design: pd.DataFrame = None, cad_data: dict = None) -> pd.DataFrame:
        """
        ...
        # Design: Code=Col I (8), Name=Col A (0)
        design_caps = {} 
        design_name_to_code = {} # New: Map Name -> Code (Col A -> Col I)
        
        if df_design is not None and not df_design.empty:
             has_box_code = len(df_design.columns) > 8
             ...
             for _, row in df_design.iterrows():
                 try:
                     ...
                     # Store by Code (Col I)
                     if has_box_code:
                         c_val = str(row.iloc[8]).strip().upper()
                         if c_val and c_val != 'NAN': 
                             design_caps[c_val] = d_cap
                         
                     # Store by Name (ONLY Column B / Index 1 for MN design)
                     if len(row) > 1:
                         n_val2 = str(row.iloc[1]).strip().upper()
                         if n_val2 and n_val2 != 'NAN':
                             design_caps[n_val2] = d_cap
                             # Map Name -> Code
                             if c_val and c_val != 'NAN':
                                 design_name_to_code[n_val2] = c_val
                 except: pass

        # --- PROCESS CAD DATA IF AVAILABLE ---
        if cad_data:
            for cab_name, info in cad_data.items():
                cab_key = str(cab_name).strip().upper()
                c_num = info.get('capacity_num', 0)
                c_code = info.get('box_code', '')
                
                if c_num > 0:
                    design_caps[cab_key] = c_num
        ...
        for _, row in df_bbnt_dt.iterrows():
            ...
            d_cap = design_caps.get(obj_key, 0)
            
            # Import Cap is now a tuple (cap, label)
            i_data = import_caps.get(obj_key, (0, ""))
            ...
            design_code = design_name_to_code.get(obj_key)
            import_code = import_name_to_code.get(obj_key)
            
            has_m_box = False
            # Check if it is a măng xông/manhole (ends with /MO or /MX)
            if obj_key.endswith('/MO') or obj_key.endswith('/MX'):
                has_m_box = True
            else:
                for code in [design_code, import_code]:
                    if code:
                        code_upper = str(code).upper().strip()
                        # Product codes starting with MO or MX
                        if code_upper.startswith('MO') or code_upper.startswith('MX'):
                            has_m_box = True
                            break
                        # Product codes starting with M followed by a digit (e.g. M12, M24) but not starting with standard province prefixes
                        if code_upper.startswith('M') and len(code_upper) > 1 and code_upper[1].isdigit():
                            is_province = any(code_upper.startswith(pref) for pref in ['HCM', 'HNI', 'CMU', 'HPG', 'HDG', 'STG', 'LAN', 'BDG', 'DNA', 'VTU'])
                            if not is_province:
                                has_m_box = True
                                break

            # 3. Capacity Cross Check
            is_ignored_type = any(obj_key.endswith(suffix) for suffix in ['/FO', '/TO', '/TU'])
    ...
    def check_han_noi(self, df_tk: pd.DataFrame, df_bbnt_hannoi: pd.DataFrame, df_imp: pd.DataFrame = None) -> pd.DataFrame:
        ...
        2.Hàm check_han_noi (Updated):
           ...
           - SL Thiết kế: So khớp Vị trí với Cột B file list Thiết Kế, lấy Cột F file Thiết Kế.
        ...
        # Ánh xạ Thiết kế (Cột B -> Cột F)
        dict_tk = {}
        tk_positions_raw = {} # Để lưu tên gốc phục vụ hiển thị nếu thiếu
        if not df_tk.empty and len(df_tk.columns) > 5:
            c_tk_key = df_tk.columns[1]   # Cột B
            c_tk_val = df_tk.columns[5]   # Cột F
    ...
    def check_tuyen_cap(self, df_tk: pd.DataFrame, df_tuyencap: pd.DataFrame, df_imp_cap: pd.DataFrame = None) -> pd.DataFrame:
        ...
        3. Hàm check_tuyen_cap (Updated):
           ...
           - Comparisons:
              1. Dung lượng: Col F (BBNT) vs Col D (TK) - lấy số.
              2. Loại cáp: Col G (BBNT) vs Col D (TK) - lấy phần chữ. FO=Treo(CT), DU=Ngầm(CC).
        ...
        # --- MATCHING DESIGN AND BBNT SEGMENTS WITHOUT Cartesian DUPLICATION ---
        results = []
        for _, row in df_bbnt.iterrows():
            tuyen_name = row[col_tuyen_bbnt]
            if pd.isna(tuyen_name) or str(tuyen_name).strip() == '':
                continue
                
            start_val_raw = row[col_start_bbnt]
            start_val = extract_endpoint(start_val_raw) if pd.notna(start_val_raw) and str(start_val_raw).strip() != '' else ""
            end_val_raw = row[col_end_bbnt]
            end_val = extract_endpoint(end_val_raw) if pd.notna(end_val_raw) and str(end_val_raw).strip() != '' else ""
            
            # Find matching row in df_tk (Design)
            match_row = None
            if df_tk is not None and not df_tk.empty:
                # 1. Try exact undirected segment match (start <-> start and end <-> end)
                for _, tk_row in df_tk.iterrows():
                    tk_start = extract_endpoint(tk_row.iloc[0]) if len(tk_row) > 0 else ""
                    tk_end = extract_endpoint(tk_row.iloc[1]) if len(tk_row) > 1 else ""
                    
                    if (start_val == tk_start and end_val == tk_end) or (start_val == tk_end and end_val == tk_start):
                        match_row = tk_row
                        break
                
                # 2. Try fallback match by end_val only (matches either tk_end or tk_start)
                if match_row is None:
                    for _, tk_row in df_tk.iterrows():
                        tk_start = extract_endpoint(tk_row.iloc[0]) if len(tk_row) > 0 else ""
                        tk_end = extract_endpoint(tk_row.iloc[1]) if len(tk_row) > 1 else ""
                        
                        if end_val == tk_end or end_val == tk_start:
                            match_row = tk_row
                            break
            
            errors = []
            
            # Extract Design values if match found
            cap_tk = 0
            type_tk_raw = ""
            len_tk = 0
            has_design_match = (match_row is not None)
            
            if has_design_match:
                cap_tk = self._safe_num(match_row.iloc[3]) if len(match_row) > 3 else 0
                type_tk_raw_full = str(match_row.iloc[3]).strip() if len(match_row) > 3 else ""
                type_tk_raw = re.sub(r'\d+', '', type_tk_raw_full).strip().upper()
                # Display mapping
                if type_tk_raw == "FO": type_tk_raw = "TREO"
                elif type_tk_raw == "DU": type_tk_raw = "NGAM"
                len_tk = self._safe_num(match_row.iloc[4]) if len(match_row) > 4 else 0
            
            # 1. Capacity (F vs C)
            cap_bbnt = self._safe_num(row[col_cap_bbnt])
            if cap_bbnt != cap_tk:
                errors.append(f"Lệch Dlượng: TT={cap_bbnt} vs TK={cap_tk}")
                
            # 2. Type (G vs D)
            type_bbnt_raw = self._normalize_text(row[col_type_bbnt])
            type_tk_norm = self._normalize_text(type_tk_raw)
            
            def get_standard_type(t_raw):
                if not t_raw: return ""
                if any(x in t_raw for x in ["TREO", "CT", "CAPTREO", "FO"]): return "CT"
                if any(x in t_raw for x in ["NGAM", "NGẦM", "CC", "CAPNGAM", "CAPNGẦM", "DU"]): return "CC"
                return t_raw

            std_bbnt = get_standard_type(type_bbnt_raw)
            std_tk = get_standard_type(type_tk_norm)
            ...
            len_h_bbnt = self._safe_num(row[col_len_h_bbnt])
            len_i_bbnt = self._safe_num(row[col_len_bbnt])
            ...
            if not has_design_match:
                 status = "⚠️ Không tìm thấy TK"
                 errors = [f"Không tìm thấy đối tượng '{end_val}' trong file TK"]
            ...
            cs_dau_raw = row[col_cs_dau] if col_cs_dau in row else None
            cs_cuoi_raw = row[col_cs_cuoi] if col_cs_cuoi in row else None
            ...
            results.append({
                ...
                "Ghi chú": str(row[col_note_tc]) if col_note_tc in row and pd.notna(row[col_note_tc]) else ""
            })
