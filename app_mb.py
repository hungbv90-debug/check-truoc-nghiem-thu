# -*- coding: utf-8 -*-
"""
app_mb.py — Giao diện Streamlit Đối Soát Nghiệm Thu — MIỀN BẮC.
...
import time
import importlib
import data_processor_mb as data_processor
import cad_processor_mb as cad_processor
import streamlit.components.v1 as components
# Force reload to pick up new methods if cached
importlib.reload(data_processor)
from data_processor_mb import QALogic
...
def main():
    ...
    with st.sidebar:
        ...
        with col_back:
            if st.button("◀ Về", key="back_home_mb", use_container_width=True):
                ...
                st.rerun()
        with col_badge:
            st.markdown('<div style="background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;padding:6px 12px;border-radius:8px;text-align:center;font-weight:700;font-size:0.85rem;">🔵 MIỀN BẮC</div>', unsafe_allow_html=True)
    ...
    if is_any_loaded:
        if st.session_state.get('recalculate_results', True):
            ...
            res_doi_tuong = qa.check_doi_tuong(df_imp, df_bbnt_dt, df_std_tk) if 'doi_tuong' in present_types else pd.DataFrame()
        ...
        # 3. Update Đối tượng with CAD data
        if not res_doi_tuong.empty:
            vi_tri_col = next((c for c in res_doi_tuong.columns if c.lower() == 'đối tượng'), None)
            if vi_tri_col:
                def get_cad_box_code(key):
                    if not cad_map: return ""
                    if pd.isna(key): return ""
                    key_str = str(key).strip().upper()
                    
                    if key_str in cad_map:
                        return cad_map[key_str].get('box_code', '')
                    prefix_key = key_str.split('/')[0] if '/' in key_str else key_str
                    if prefix_key in cad_map:
                        return cad_map[prefix_key].get('box_code', '')
                        
                    p_idx = key_str.find('P')
                    if p_idx != -1:
                        k = key_str[p_idx:]
                        if k in cad_map: return cad_map[k].get('box_code', '')
                        prefix = k.split('/')[0] if '/' in k else k
                        if prefix in cad_map: return cad_map[prefix].get('box_code', '')
                    return ""
                    
                def get_cad_capacity(key):
                    if not cad_map: return ""
                    if pd.isna(key): return ""
                    key_str = str(key).strip().upper()
                    
                    if key_str in cad_map:
                        return cad_map[key_str].get('capacity_text', '')
                    prefix_key = key_str.split('/')[0] if '/' in key_str else key_str
                    if prefix_key in cad_map:
                        return cad_map[prefix_key].get('capacity_text', '')
                        
                    p_idx = key_str.find('P')
                    if p_idx != -1:
                        k = key_str[p_idx:]
                        if k in cad_map: return cad_map[k].get('capacity_text', '')
                        prefix = k.split('/')[0] if '/' in k else k
                        if prefix in cad_map: return cad_map[prefix].get('capacity_text', '')
                    return ""

                # Insert 'Dung lượng phối CAD' after 'Dung lượng (Thiết kế/Import)'
                dl_col = next((c for c in res_doi_tuong.columns if 'dung lượng' in c.lower()), None)
                if dl_col:
                    idx_dl = res_doi_tuong.columns.get_loc(dl_col)
                    if 'Dung lượng phối CAD' not in res_doi_tuong.columns:
                        res_doi_tuong.insert(idx_dl + 1, 'Dung lượng phối CAD', res_doi_tuong[vi_tri_col].apply(get_cad_capacity))
                        
                # Insert 'Mã hộp phối CAD' after 'Mã hộp (Thiết kế/Import)'
                mh_col = next((c for c in res_doi_tuong.columns if 'mã hộp' in c.lower()), None)
                if mh_col:
                    idx_mh = res_doi_tuong.columns.get_loc(mh_col)
                    if 'Mã hộp phối CAD' not in res_doi_tuong.columns:
                        res_doi_tuong.insert(idx_mh + 1, 'Mã hộp phối CAD', res_doi_tuong[vi_tri_col].apply(get_cad_box_code))

                # Update 'Chi tiết Lỗi khác' with CAD comparisons
                ct_col = next((c for c in res_doi_tuong.columns if 'chi tiết' in c.lower()), None)
                if ct_col:
                    def get_py_import_capacity(val):
                        if pd.isna(val): return ""
                        clean = str(val).replace('✅', '').replace('❌', '').replace('⚠️', '').strip()
                        parts = clean.split('/')
                        if len(parts) > 1:
                            return parts[1].strip().upper()
                        return clean.upper()

                    def get_py_import_box_code(val):
                        if pd.isna(val): return ""
                        clean = str(val).replace('✅', '').replace('❌', '').replace('⚠️', '').strip()
                        parts = clean.split('/')
                        if len(parts) > 1:
                    ...
                    new_dung_luong_cad = []
                    new_ma_hop_cad = []
                    for _, row in res_doi_tuong.iterrows():
                        orig_err = str(row[ct_col]).strip() if pd.notna(row[ct_col]) else ""
                        if orig_err.lower() in ["nan", "-", ""]:
                            orig_err = ""
                        errs = []
                        
                        # Compare Capacity
                        cap_cad = str(row.get('Dung lượng phối CAD', '')).strip()
                        final_cap_cad = cap_cad
                        if cap_cad and cap_cad != '-' and cap_cad != 'KHÔNG TÌM THẤY' and cap_cad.upper() != 'NAN':
                            cap_cad_clean = cap_cad.split('(')[0].strip().upper()
                            imp_cap = get_py_import_capacity(row.get(dl_col, ''))
                            if imp_cap and imp_cap != cap_cad_clean:
                                errs.append(f"Sai lệch Dung lượng so với CAD (Import: {imp_cap}, CAD: {cap_cad})")
                                final_cap_cad = f"❌ {cap_cad}"
                            else:
                                final_cap_cad = f"✅ {cap_cad}"
                        new_dung_luong_cad.append(final_cap_cad)
                                
                        # Compare Box Code
                        box_cad = str(row.get('Mã hộp phối CAD', '')).strip()
                        final_box_cad = box_cad
                        if box_cad and box_cad != '-' and box_cad != 'KHÔNG TÌM THẤY' and box_cad.upper() != 'NAN':
                            box_cad_clean = box_cad.upper()
                            imp_box = get_py_import_box_code(row.get(mh_col, ''))
                            if imp_box and imp_box != box_cad_clean:
                                errs.append(f"Sai lệch Mã hộp so với CAD (Import: {imp_box}, CAD: {box_cad})")
                                final_box_cad = f"❌ {box_cad}"
                            else:
                                final_box_cad = f"✅ {box_cad}"
                        new_ma_hop_cad.append(final_box_cad)
                                
                        if errs:
                            new_err_str = "\n".join([f"- {e}" for e in errs])
                            if orig_err:
                                final_err = f"{orig_err}\n{new_err_str}"
                            else:
                                final_err = new_err_str
                        else:
                            final_err = orig_err
                            
                        new_chi_tiet.append(final_err)
                        
                    res_doi_tuong[ct_col] = new_chi_tiet
                    res_doi_tuong['Dung lượng phối CAD'] = new_dung_luong_cad
                    res_doi_tuong['Mã hộp phối CAD'] = new_ma_hop_cad
    ...
    if nav == "Kết quả phân tích":
            ...
            if res_doi_tuong.empty:
                ...
                for idx, row in res_doi_tuong.iterrows():
                    dt_rows.append({
                        ...
                        "dung_luong_cad": str(row.get("Dung lượng phối CAD", "")),
                        "ma_hop": str(row.get("Mã hộp (Thiết kế/Import)", "")),
                        "ma_hop_cad": str(row.get("Mã hộp phối CAD", "")),
                ...
                <style>
                    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
                    ...
                    table.dt-table th:nth-child(6), table.dt-table td:nth-child(6),
                    table.dt-table th:nth-child(7), table.dt-table td:nth-child(7) {{
                        width: 120px;
                        white-space: normal;
                        word-wrap: break-word;
                    }}
                ...
                <table class="dt-table">
                    <thead>
                        <tr>
                            ...
                            <th>Dung lượng phối CAD</th>
                            <th>Mã hộp (Thiết kế/Import)</th>
                            <th>Mã hộp phối CAD</th>
                ...
                <script>
                    ...
                    function renderTable() {{
                        ...
                        dataToRender.forEach(r => {{
                            ...
                            const allContent = r.doi_tuong + r.power + r.dung_luong + r.dung_luong_cad + r.ma_hop + r.ma_hop_cad + r.chi_tiet;
                            ...
                            tr.innerHTML = `
                                ...
                                <td ${{getCellClass(r.dung_luong_cad)}}>${{r.dung_luong_cad}}</td>
                                <td ${{getCellClass(r.ma_hop)}}>${{r.ma_hop}}</td>
                                <td ${{getCellClass(r.ma_hop_cad)}}>${{r.ma_hop_cad}}</td>
                    ...
                    function renderTable() {{
                        ...
                        dataToRender.forEach(r => {{
                            ...
                            tr.innerHTML = `
                                ...
                                <td ${{getCellClass(r.cad_diem_dau)}}>${{r.cad_diem_dau}}</td>
        ...
        with t3:
            if res_han_noi.empty:
                ...
                <p class="hn-hint"><b>Tổng đề nghị: {sum_dn}, Tổng thiết kế: {sum_tk}{diff_text}</b></p>
                    ...
                    function renderTable() {{
                        ...
                        dataToRender.forEach(r => {{
                            ...
                            tr.innerHTML = `
                                ...
                                <td ${{getStatusCellClass(r.cad_sl_han)}}>${{r.cad_sl_han}}</td>
